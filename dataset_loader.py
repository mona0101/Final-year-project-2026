...
class DroneFusionDataset(Dataset):
    def __init__(
        ...
        duration=10,
        segment_length=0.25,
        audio_feature_type='mfcc',  # 'mfcc' or 'logmel'
        audio_sr=None               # <-- new optional parameter
    ):
        ...
        self.audio_sr = audio_sr  # <-- store user-defined sample rate
    ...
    def __getitem__(self, idx):
        ...
        # ------------------------
        if self.audio_sr is not None:
            sr = self.audio_sr  # use user-defined value
        else:
            sr = 16000 if self.audio_feature_type == 'logmel' else 44100
        ...
        # ------------------------
        # Waveform normalization only for log-mel
        # ------------------------
        ...
        video_tensor = torch.stack([self.transform['video'](f) for f in video_frames]) if 'video' in self.transform and video_frames else torch.empty(0)
        rf_tensor = torch.stack([self.transform['rf'](f) for f in rf_frames]) if 'rf' in self.transform and rf_frames else torch.empty(0)
